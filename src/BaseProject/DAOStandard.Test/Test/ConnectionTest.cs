...
public class ConnectionTest
{
    ...
    /// </summary>
    [Fact]
    public void CloseConnectionAsync_ConnectionCloseSuccessfully_ReturnsSuccessfulResult()
    {
        var (mockDbConnection, databaseDAO) = SetupMock.SetupDatabaseDAO();
        //配置模擬連線物件
        ...
        // Act
        var result =  databaseDAO.CloseConnection();
    ...
    /// </summary>
    [Fact]
    public void CloseConnectionAsync_ConnectionFails_ReturnsFailedResult()
    {
        ...
        var (mockDbConnection, databaseDAO) = SetupMock.SetupDatabaseDAO();
        //配置模擬連線物件
        mockDbConnection.Setup(m => m.Close()).Throws(new Exception(exMessage));
        mockDbConnection.Setup(m => m.State).Returns(ConnectionState.Open);
        //設置模擬物件到主方法
        databaseDAO.SetMockConnection(mockDbConnection.Object);

        // Act
        var result = Assert.Throws<ConnectionException>(databaseDAO.CloseConnection);
