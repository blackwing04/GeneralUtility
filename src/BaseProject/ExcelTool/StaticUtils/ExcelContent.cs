...
namespace ExcelTool.StaticUtil
{
    ...
    public static class ExcelContent
    {
        ...
        /// <param name="excelMapper">Excel內容驗證與名稱映射的設定。</param>
        /// <param name="skipFirstRow">跳過第一行標題行</param>
        private static DataRow? CreateDataRow(IXLRow row, ImportModel importModel, ExcelMapperSetting excelMapper, bool skipFirstRow)
        {
            var dataRow = importModel.ResultTable.NewRow();
            ExcelRowProcessingModel processingModel = new (row);
            bool isValidRow = true;
            int excelColumnIndex = processingModel.FirstExcelColumnIndex;
            // 用於緩存欄裡每一個儲存格內容的字典
            Dictionary<int, List<string>> columnValuesCache = new();
            foreach (IXLCell cell in row.Cells(processingModel.FirstExcelColumnIndex, processingModel.LastExcelColumnIndex)) {
                try {
                    ...
                    LogRowError(row, importModel.ErrorTable, processingModel, ex);
        ...
        /// <param name="errorTable">要填充的錯誤資料表</param>
        /// <param name="processingModel">發生錯誤的儲存格值</param>
        /// <param name="ex">發生的具體錯誤</param>
        private static void LogRowError(IXLRow row, DataTable errorTable
            , ExcelRowProcessingModel processingModel, Exception ex)
        {
            ...
            foreach (IXLCell cell in row.Cells(processingModel.FirstExcelColumnIndex, processingModel.LastExcelColumnIndex)) {
                ...
                if (dtColumnIndex >= errorTable.Columns.Count - 1)//錯誤表多一欄錯誤訊息
                    break;
