...
namespace ExcelTool.StaticUtil
{
    public static class ExcelHeader
    {
        ...
        public static ImportModel InitializeDataTable(ExcelMapperSetting excelMapper)
        {
            ...
            return new() { ResultTable = resultTable, ErrorTable = errorTable };
        }
        #endregion 初始化方法

        #region 取得Excel表頭
        /// <summary>
        /// 從指定的 IXLWorksheet 中獲取第一行使用中的儲存格並轉為字串列表
        /// </summary>
        /// <param name="workSheet">要處理的 IXLWorksheet</param>
        /// <param name="firstRowIsHeader">第一行為標題行</param>
        /// <returns>第一行使用中的儲存格內容列表，或者 Excel 預設的列標題</returns>
