...
namespace ExcelTool.StaticUtil.Models
{
    #region 轉換匯入模型
    ...
    public class ExcelRowProcessingModel
    {
        ...
        /// 有使用的Excel欄位第一個索引
        /// </summary>
        public int FirstExcelColumnIndex { get; set; }
        /// <summary>
        /// 有使用的Excel欄位的最後一個索引
        /// </summary>
        public int LastExcelColumnIndex { get; set; }

        public ExcelRowProcessingModel(IXLRow row)
        {
            FirstExcelColumnIndex = row.FirstCellUsed()?.Address.ColumnNumber ?? 0;
            LastExcelColumnIndex = row.LastCellUsed()?.Address.ColumnNumber ?? 0;
        }
